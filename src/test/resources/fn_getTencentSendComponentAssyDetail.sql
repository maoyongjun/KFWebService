...
ALTER function [dbo].[fn_getTencentSendComponentAssyDetail]
(
	@SN	VARCHAR(MAX)
)
returns
...
BEGIN
	...
	SELECT	a.sysserialno SN ,b.CONTROLVALUE ctype,A.partno PN,A.cserialno  csn,CONTROLDESC AS descriptions,
			ROW_NUMBER() over(PARTITION BY a.sysserialno,b.CONTROLDESC order by b.CONTROLDESC,CASE WHEN b.CONTROLDESC=N'硬盘' THEN a.cserialno ELSE right(A.cserialno,8 ) END) as num,A.categoryname
	...
	WHERE	categoryname IN ('MG06','MG28FC','MG28IA','MG28LED','MG28SW',
			...
			AND sysserialno IN (SELECT VALUE FROM dbo.fn_Split(@SN,',')) 
	...
	SELECT 	A.sysserialno SN ,D.CONTROLVALUE ctype,C.partno PN,C.cserialno  csn,CONTROLDESC AS descriptions,
			ROW_NUMBER() over(PARTITION BY A.sysserialno,d.CONTROLDESC order by d.CONTROLDESC,CASE WHEN d.CONTROLDESC=N'硬盘' THEN C.cserialno ELSE  right(C.cserialno,8) END) as num ,c.categoryname
	FROM 	mfworkstatus A, mfsyscserial B,mfsyscserial C ,ECONTROLVALUE D
	WHERE 	A.sysserialno IN (SELECT VALUE FROM dbo.fn_Split(@SN,',')) 	
	...
	INSERT INTO @assyData
	SELECT T2.SN,T2.ctype,T2.PN,T2.csn,T2.descriptions,ROW_NUMBER() over(PARTITION BY T2.SN,T2.ctype order by t2.ctype,right(CSN,8)) as num,T2.categoryname  FROM (
